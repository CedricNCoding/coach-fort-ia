...
-- Create a function to get the encryption key from environment
-- This key should be set in Supabase project settings
...
-- Create a secure function to store encrypted API key
...
BEGIN
  INSERT INTO public.ai_settings (
    ...
    _user_id,
    encode(pgp_sym_encrypt(_api_key, get_encryption_key()), 'base64'),
    _model_name,
    _base_url,
    _user_role,
    _user_needs
  ...
  DO UPDATE SET
    api_key = CASE 
      ...
      THEN encode(pgp_sym_encrypt(_api_key, get_encryption_key()), 'base64')
...
-- Create a secure function to get decrypted API key (only for server use)
...
-- Update RLS policy to never expose the encrypted api_key to clients
DROP POLICY IF EXISTS "Users can manage their ai settings" ON public.ai_settings;

CREATE POLICY "Users can view their ai settings (without api_key)"
ON public.ai_settings
FOR SELECT
TO authenticated
USING (user_id = auth.uid());

-- Note: INSERT and UPDATE will be handled through the set_user_api_key function
-- but we keep policies for other fields
CREATE POLICY "Users can manage their ai settings metadata"
ON public.ai_settings
FOR ALL
TO authenticated
USING (user_id = auth.uid())
