...
-- The get_encryption_key function doesn't need search_path since it doesn't access tables

-- Recreate get_user_api_key with proper search_path
...
-- Recreate set_user_api_key with proper search_path
...
BEGIN
  INSERT INTO public.ai_settings (
    ...
    _user_id,
    encode(pgp_sym_encrypt(_api_key, public.get_encryption_key()), 'base64'),
    _model_name,
    _base_url,
    _user_role,
    _user_needs
  ...
  DO UPDATE SET
    api_key = CASE 
      ...
      THEN encode(pgp_sym_encrypt(_api_key, public.get_encryption_key()), 'base64')
